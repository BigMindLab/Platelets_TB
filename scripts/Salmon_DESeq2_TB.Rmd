...
blacclist_noisy <- data.frame(read.csv("blacklist_noisy_mean_normalized_leq25.tsv", sep = "\t", header = TRUE))
...
low_counts_cutoff <- c(2)

genes.low_counts <- rownames(normalized.counts[normalized.counts$Mean <= low_counts_cutoff,])

# Check if there is genes in lowcount less then the normalized counts df
length(genes.low_counts)
nrow(normalized.counts)

# Add porcentage of low counts
100*length(genes.low_counts) / nrow(normalized.counts)
txi.toGene <- summarizeToGene(txi, tx2gene[, c("transcriptID","geneID")])
...
write.xlsx(normalized.counts.annotated, file = "Results_v1/Normalized_counts_cleaned.xlsx", colNames = T, rowNames = F, append = F, overwrite = T)
...
ggsave("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Plots_v1/heatmap/Heatmap_samples_TB_Control_noncommon_genes_log2FC_meq_2_(padj_0.05)_WPGMA.pdf",
       plot = heatmap_genes, width = 6, height = 9, dpi = 300, units = "in")
...
expression_data_TB <- expression_data_TB[, c(21, 27, 1:19)]
...
write.table(expression_data_TB, file = "Results_v1/GSEA_imput/expression_data_TB.txt",
