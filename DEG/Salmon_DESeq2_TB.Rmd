...
# comando to see version of org.HS.eg.db library
...
sampledata <- sampledata[, c("id", "num", "group", "sex", "age", "RIN_ours", "RIN_Ed")]
...
### [RUN ONCE] Create noisy genes blacklist
```{r}
counts.gene_annotations$MEAN <- rowMeans(counts.gene) 
blacklist_noisy <- counts.gene_annotations[counts.gene_annotations$MEAN <= 25, c("geneID", "symbol", "biotype", "MEAN")]
nrow(blacklist_noisy)
write.table(blacklist_noisy, "blacklist_noisy_mean_normalized_leq25.tsv", sep = "\t", col.names = T, row.names = F)
```
...
```{r}
blacklist_nRibo <- data.frame(read.csv("../DEG/Blacklists/blacklist_nuclear-ribo.tsv", sep = "\t", header = TRUE))
blacklist_mtRibo <- data.frame(read.csv("../DEG/Blacklists/blacklist_mito-ribo.tsv", sep = "\t", header = TRUE))
blacklist_allRibo <- data.frame(read.csv("../DEG/Blacklists/blacklist_all-ribo.tsv", sep = "\t", header = TRUE))
blacklist_allLeuko <- data.frame(read.csv("../DEG/Blacklists/blacklist_all-Leuko.tsv", sep = "\t", header = TRUE))
blacklist_noisy <- data.frame(read.csv("../DEG/Blacklists/blacklist_noisy_mean_normalized_leq25.tsv", sep = "\t", header = TRUE))
...
# Important step to filter noise and leucyte genes
genecounts.filtered <- counts.gene[!(rownames(counts.gene) %in% c(blacklist_allRibo[,1], blacklist_allLeuko[,1], blacklist_noisy[blacklist_noisy$MEAN <= 2, 1])), ]


# Number of genes in noyse blacklist
length(unique(blacklist_noisy[blacklist_noisy$MEAN <= 2, 1])) ## = 17644

# See percentage of number of genes removed
100 - 100*nrow(genecounts.filtered) / nrow(counts.gene)

# See percentage of gene counts removed
100 - 100*sum(genecounts.filtered) / sum(counts.gene)

# Add counts to sampledata
...
## Filter out noise blacklist
```{r}
# Step 1: Filter genes with MEAN <= 2
low_expression_genes_25 <- blacklist_noisy[blacklist_noisy$MEAN <= 25, 1]
low_expression_genes_25 <- setdiff(low_expression_genes_25, c(blacklist_allRibo[,1], blacklist_allLeuko[,1]))  # Exclude permitted genes
cat("Number of genes removed (MEAN <= 25):", length(low_expression_genes_25), "\n")

genecounts_MNC_25 <- gene.counts[low_expression_genes_25, ]  # Filter

# Function to progressively filter genes
filter_genes <- function(previous_genes, cutoff, previous_counts) {
  current_genes <- blacklist_noisy[blacklist_noisy$MEAN <= cutoff, 1]
  current_genes <- intersect(current_genes, rownames(previous_counts))  # Only consider remaining genes
  current_genes <- unique(current_genes)
  cat("Number of genes removed (MEAN <=", cutoff, "):", length(current_genes), "\n")
  return(previous_counts[current_genes, ])
}

# Stepwise filtering using the function
genecounts_MNC_10   <- filter_genes(low_expression_genes_25, 10, genecounts_MNC_25)
genecounts_MNC_7    <- filter_genes(low_expression_genes_10, 7, genecounts_MNC_10)
genecounts_MNC_6    <- filter_genes(low_expression_genes_10, 6, genecounts_MNC_7)
genecounts_MNC_5    <- filter_genes(low_expression_genes_10, 5, genecounts_MNC_6)
genecounts_MNC_4    <- filter_genes(low_expression_genes_10, 4, genecounts_MNC_5)
genecounts_MNC_3    <- filter_genes(low_expression_genes_5, 3, genecounts_MNC_4)
genecounts_MNC_2    <- filter_genes(low_expression_genes_3, 2, genecounts_MNC_3)
genecounts_MNC_1_5  <- filter_genes(low_expression_genes_2, 1.5, genecounts_MNC_2)
genecounts_MNC_1    <- filter_genes(low_expression_genes_1_5, 1, genecounts_MNC_1_5)


# Remove the rows where rownames are in exclude
gene.counts_filtered <- gene.counts[!(rownames(gene.counts) %in% rownames(genecounts_MNC_2)), ]
gene.counts_filtered <- gene.counts_filtered [!(rownames(gene.counts_filtered) %in% blacklist_allLeuko[,1]), ]
gene.counts_filtered <- gene.counts_filtered [!(rownames(gene.counts_filtered) %in% blacklist_allRibo[,1]), ]

```

### Add noisy genes into sampledata
```{r}

length(blacklist_noisy[blacklist_noisy$MEAN<=1, 1])
sampledata$counts_nRibo <- colSums(genecounts.nRibo[1:(length(genecounts.nRibo))])
sampledata$counts_mtRibo <- colSums(genecounts.mtRibo[1:(length(genecounts.mtRibo))])
sampledata$counts_leuko <- colSums(genecounts.leuko[1:(length(genecounts.leuko))])

100*sampledata$counts_leuko/sampledata$gene_counts

#sampledata$counts_noisy <- colSums(genecounts_MNC_25[1:(length(genecounts_MNC_25))])  
sampledata$counts_MNC_10 <- colSums(genecounts_MNC_10[1:(length(genecounts_MNC_10))])  - sampledata$counts_MNC_7
sampledata$counts_MNC_7 <- colSums(genecounts_MNC_5[1:(length(genecounts_MNC_5))])  - sampledata$counts_MNC_6
sampledata$counts_MNC_6 <- colSums(genecounts_MNC_5[1:(length(genecounts_MNC_5))])  - sampledata$counts_MNC_5
sampledata$counts_MNC_5 <- colSums(genecounts_MNC_5[1:(length(genecounts_MNC_5))])  - sampledata$counts_MNC_3
sampledata$counts_MNC_3 <- colSums(genecounts_MNC_3[1:(length(genecounts_MNC_3))])  - sampledata$counts_MNC_2
sampledata$counts_MNC_2 <- colSums(genecounts_MNC_2[1:(length(genecounts_MNC_2))])  - sampledata$counts_MNC_1_5
sampledata$counts_MNC_1_5 <- colSums(genecounts_MNC_1_5[1:(length(genecounts_MNC_1_5))]) - sampledata$counts_MNC_1
sampledata$counts_MNC_1 <- colSums(genecounts_MNC_1[1:(length(genecounts_MNC_1))])

sampledata$counts_filtered <- colSums(gene.counts_filtered[1:(length(gene.counts_filtered ))]) - sampledata$counts_MNC_10
```
...
```{r}
# CONVERT in long format for plotting the sample data
sampledata_temp <- sampledata[, c("id", "counts_nRibo", "counts_mtRibo", "counts_leuko", "counts_filtered",
                                  "counts_MNC_10", "counts_MNC_7", "counts_MNC_6", "counts_MNC_5", "counts_MNC_3",
                                  "counts_MNC_2", "counts_MNC_1_5", "counts_MNC_1")]

# Convert to long format
sampledata_long <- sampledata_temp %>%
  pivot_longer(cols = -id,  # Keep "id" column fixed
               names_to = "Condition",  # New column for variable names
               values_to = "Counts")  # New column for values

# Define the correct order for Condition
condition_order <- c("counts_nRibo", "counts_mtRibo", "counts_leuko", 
                     "counts_MNC_10", "counts_MNC_7", "counts_MNC_6", "counts_MNC_5",
                     "counts_MNC_3", "counts_MNC_2", "counts_MNC_1_5", "counts_MNC_1", "counts_filtered")

# Convert Condition to an ordered factor
sampledata_long$Condition <- factor(sampledata_long$Condition, levels = condition_order)


# Plot the counts per list STACKED into a barplot per sample 
p.counts <- ggplot(data = sampledata_long,
                   aes(x = factor(id, levels = sampledata$id), y = Counts, fill = Condition)) +
  theme_bw() + theme(panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank()) +
...
# save 
ggsave("plots_v1/Counts_with cut-offs_per_Sample.jpg", p.counts, width = 10, height = 8, dpi = 300)

my_colors <- c("#FFDEAD", "#ffb6c1", "darkred", "azure", "peru", "#F67280", "#C06C84", "#355C7D", "#F8B195", "peru", "#F67280"#
,"azure")
               
               
for (bl in c("counts_nRibo", "counts_mtRibo", "counts_leuko", "counts_MNC_10","counts_MNC_7","counts_MNC_6",
             "counts_MNC_5","counts_MNC_4","counts_MNC_3","counts_MNC_2","counts_MNC_1_5","counts_MNC_1","counts_filtered")) {
  ...
  ggsave(paste0("plots_v1/Counts_with_cutoffs_per_Sample_", unlist(strsplit(bl, split = "_"))[2], ".jpg"),
...
load("C:/Users/Dan_1/Documents/Projects/TB/Results_v1/02_16_updated_Rdata.RData")
```
...
### [RUN once] TPM of nosy and cleaned genes
```{r}
# Step 1: Filter genes with MEAN <= 25
low_expression_genes_25 <- blacklist_noisy[blacklist_noisy$MEAN <= 25, 1]
low_expression_genes_25 <- setdiff(low_expression_genes_25, c(blacklist_allRibo[,1], blacklist_allLeuko[,1]))  # Exclude permitted genes
cat("Number of genes removed (MEAN <= 25):", length(low_expression_genes_25), "\n")

geneTPM_MNC_25 <- gene.tpm[low_expression_genes_25, ]  # Filter

# Function to progressively filter genes
filter_genes <- function(previous_genes, cutoff, previous_tpm) {
  current_genes <- blacklist_noisy[blacklist_noisy$MEAN <= cutoff, 1]
  current_genes <- intersect(current_genes, rownames(previous_tpm))  # Only consider remaining genes
  current_genes <- unique(current_genes)
  cat("Number of genes removed (MEAN <=", cutoff, "):", length(current_genes), "\n")
  return(previous_tpm[current_genes, ])
}

# Stepwise filtering using the function
geneTPM_MNC_10   <- filter_genes(low_expression_genes_25, 10, geneTPM_MNC_25)
geneTPM_MNC_5    <- filter_genes(low_expression_genes_10, 5, geneTPM_MNC_10)
geneTPM_MNC_3    <- filter_genes(low_expression_genes_5, 3, geneTPM_MNC_5)
geneTPM_MNC_2    <- filter_genes(low_expression_genes_3, 2, geneTPM_MNC_3)
geneTPM_MNC_1_5  <- filter_genes(low_expression_genes_2, 1.5, geneTPM_MNC_2)
geneTPM_MNC_1    <- filter_genes(low_expression_genes_1_5, 1, geneTPM_MNC_1_5)


# Remove the rows where rownames are in exclude
gene.tpm_filtered <- gene.tpm[!(rownames(gene.tpm) %in% rownames(geneTPM_MNC_25)), ]
gene.tpm_filtered <- gene.tpm_filtered [!(rownames(gene.tpm_filtered) %in% blacklist_allLeuko[,1]), ]
gene.tpm_filtered <- gene.tpm_filtered [!(rownames(gene.tpm_filtered) %in% blacklist_allRibo[,1]), ]

### Add TPM noisy genes into sampledata


length(blacklist_noisy[blacklist_noisy$MEAN<=1, 1])
sampledata$TPM_nRibo <- colSums(geneTPM.nRibo[1:(length(geneTPM.nRibo))])
sampledata$TPM_mtRibo <- colSums(geneTPM.mtRibo[1:(length(geneTPM.mtRibo))])
sampledata$TPM_leuko <- colSums(geneTPM.leuko[1:(length(geneTPM.leuko))])

sampledata$TPM_noisy <- colSums(geneTPM_MNC_25[1:(length(geneTPM_MNC_25))])  
sampledata$TPM_MNC_10 <- colSums(geneTPM_MNC_10[1:(length(geneTPM_MNC_10))])  - sampledata$TPM_MNC_5
sampledata$TPM_MNC_5 <- colSums(geneTPM_MNC_5[1:(length(geneTPM_MNC_5))])  - sampledata$TPM_MNC_3
sampledata$TPM_MNC_3 <- colSums(geneTPM_MNC_3[1:(length(geneTPM_MNC_3))])  - sampledata$TPM_MNC_2
sampledata$TPM_MNC_2 <- colSums(geneTPM_MNC_2[1:(length(geneTPM_MNC_2))])  - sampledata$TPM_MNC_1_5
sampledata$TPM_MNC_1_5 <- colSums(geneTPM_MNC_1_5[1:(length(geneTPM_MNC_1_5))]) - sampledata$TPM_MNC_1
sampledata$TPM_MNC_1 <- colSums(geneTPM_MNC_1[1:(length(geneTPM_MNC_1))])

sampledata$TPM_filtered <- colSums(gene.tpm_filtered[1:(length(gene.tpm_filtered))]) - sampledata$TPM_noisy

write.xlsx(sampledata, file = "samples_with_TPM.xlsx", colNames = T, rowNames = F, append = F, overwrite = T)
```
...
#subgroups <- c("TPM_nRibo", "TPM_mtRibo", "TPM_leuko","TPM_MNC_1","TPM_MNC_1_5",
#               "TPM_MNC_2", "TPM_MNC_3", "TPM_MNC_5", "TPM_MNC_10",
#               "TPM_MNC_25","TPM_filtered")
subgroups <- c("TPM_nRibo", "TPM_mtRibo", "TPM_leuko","TPM_noisy","TPM_filtered")
sampledata_temp <- sampledata[, c("id", subgroups)]
# Convert to long format using pivot_longer
sampledata_TPMlong <- sampledata_temp %>%
  pivot_longer(cols = all_of(subgroups),  # Select columns to reshape
               names_to = "Condition",   # New column for variable names
               values_to = "TPM")        # New column for values
...
sampledata_TPMlong <- sampledata_TPMlong %>%
  mutate(group = 
           case_when(id %in% sampledata$id[sampledata$group == "Control"] ~ "Control",
                           TRUE ~ "TB")) %>%
  mutate(group = factor(group, levels = c("Control", "TB"), ordered = TRUE))

names(sampledata_TPMlong)[names(sampledata_TPMlong) == "Condition"] <- "Gene"

#sampledata_TPMlong$Gene_Type <- factor(sampledata_TPMlong$Gene_Type, levels = c("TPM_nRibo","TPM_mtRibo", "TPM_leuko", "TPM_MNC_1", "TPM_MNC_1_5", "TPM_MNC_2", "TPM_MNC_3", "TPM_MNC_5", "TPM_MNC_10","TPM_MNC_25", "TPM_filtered"))

#sampledata_TPMlong$Gene <- factor(sampledata_TPMlong$Gene, levels = c("TPM_nRibo","TPM_mtRibo", "TPM_leuko", "TPM_noisy", "TPM_filtered"))

# Change names of gene column
sampledata_TPMlong <- sampledata_TPMlong %>%
  mutate(Gene = case_when(Gene == "TPM_nRibo" ~ "Nuclear Ribosome",
                          Gene == "TPM_mtRibo" ~ "Mothocondrial Ribosome",
                          Gene == "TPM_leuko" ~ "Leukocytes",
                          Gene == "TPM_noisy" ~ "Noise",
                          Gene == "TPM_filtered" ~ "Cleaned"))

# Factor gene
sampledata_TPMlong$Gene <- factor(sampledata_TPMlong$Gene, levels = c("Nuclear Ribosome", "Mothocondrial Ribosome", "Leukocytes", "Noise", "Cleaned"))

sampledata_TPMlong[sampledata_TPMlong$id == "114",c("TPM", "Gene")]
unique(sampledata_TPMlong$Gene)

bl.colors <- c("#FF6F61", "#FFD166","darkred", "#6B5B95", "azure", "peru", "#F67280", "#C06C84", "#355C7D", "#F8B195","azure")# "#A2D5F2")"#88D8B0",
c("coral1", "gold", "peru", "azure", "blue", "darkblue", "darkgreen", "green", , "red", "#FFDEAD")
p.TPMs <- ggplot(data = sampledata_TPMlong,
                 aes(x = factor(id, levels = sampledata$id), y = TPM, fill = Gene)) +
  theme(
...
ggsave("plots_v1/TPM_oficial/TPMs_per_Sample_subgroups.jpg", p.TPMs, width = 10, height = 8, dpi = 300)
...
keep <- rownames(txi.toGene$counts) %in% rownames(genecounts.filtered)
table(keep)
...
normalized.counts$Mean <- rowMeans(normalized.counts[,])
...
# Save the normalized counts into a tsv format 
write.table(normalized.counts.annotated, "../DEG/Results_v2/04_02-normalized_gene_counts_annotated.tsv", sep = "\t", col.names = T, row.names = F)
...
normCounts$category <- ifelse(normCounts$Mean == 0, "= 0",
                              ifelse(normCounts$Mean <= 1, "<= 1",
                                      ifelse(normCounts$Mean <= 1.5, "<= 1.5",  
                                             ifelse(normCounts$Mean <= 2, "<= 2",
                                                    ifelse(normCounts$Mean <= 3, "<= 3",
                                                           ifelse(normCounts$Mean <= 5, "<= 5",
                                                                   ifelse(normCounts$Mean <= 6, "<= 6",
                                                                           ifelse(normCounts$Mean <= 7, "<= 7",
                                                                                  ifelse(normCounts$Mean <= 10, "<= 10",
                                                                                         ifelse(normCounts$Mean <= 25, "<= 25", "> 25"))))))))))

normCounts$category <- factor(normCounts$category, levels = c("= 0", "<= 1", "<= 1.5", "<= 2", "<= 3", "<= 5","<= 6","<= 7", "<= 10", "<= 25", "> 25"))

# Barplot per sample

p.bar <- ggplot(data = normCounts, aes(x = category, fill = category)) +
  geom_bar() +
  theme_bw() +
...
p.bar
#save 
ggsave("plots_v1/Barplot_cut-offs-Normalized_Counts.jpg", p.bar, width = 10, height = 8, dpi = 300)

# Histogram
histogram_normcount <- ggplot(normCounts[normCounts$Mean <= 30, ],
                              aes(x = Mean, fill = category)) +
  geom_histogram(binwidth = 1, color = "black", position = "stack") +
  theme_bw() +
  xlab("Mean Normalized Counts") +
  ylab("Frequency") +
  scale_x_continuous(breaks = seq(0, 30, by = 5), limits = c(0,30), oob = scales::squish) + #, oob = scales::squish
  scale_y_continuous(breaks = seq(0, 2500, by = 250), limits = c(0,2500), oob = scales::squish) + #
  scale_fill_manual(values = c("= 0" = "grey70",
                               "<= 1" = "coral1",
                               "<= 1.5" = "#6B8E23",
                               "<= 2" = "gold",
...
# Save
ggsave("plots_v1/Histogram_Normalized_Counts_v5_zoom.jpg", histogram_normcount, width = 10, height = 9, dpi = 300)
```
...
confounding_var <- "group"
confounding_name <- c("group")
...
for (i in seq_along(confounding_var)) {
  pca12_clust <- PCA_clust(
    ...
    colors = c("#bbfffeff", "#ffb6c1ff","#ffa07a", "#ffe4b5", "#98fb98", "#dda0dd"), #my_colors,
    shapes = 21:22,
    legend_title = 15,
    legend_elements = 14,
    legend_pos = NULL,
  ...
  ggsave(filename = paste0("plots_v1/dim_reduction_PCA_", confounding_var[i], "_wo_lowCounts_co_2.jpg"),
         plot = pca12_clust, width = 9, height = 6, dpi = 300)
}
...
lapply(heatmap_configs, function(config) {
  ...
  ggsave(paste0("plots_v1/dim_reduction/Heatmaps/", config$filename, ".jpg"),
         plot = heatmap_samples, width = 7, height = 7, dpi = 300)
})
```
...
res.TB_C <- results(ds.deseq, name = "group_TB_vs_Control",
                    ...
                    alpha = cutoff_alpha,
                    #independentFilterimeng = TRUE, # Automatic filtering
...
# Create the dataset
screening_DEseq <- data.frame(
  cut_off = c("0", "≤1", "≤1.5", "≤2", "≤3", "≤4", "≤5", "≤6", "≤7", "≤10"),  # Treat as categorical
  LFC_up = c(839, 852, 851, 843, 840, 833, 814, 811, 804, 778),   # LFC > 0
  LFC_down = c(464, 465, 458, 439, 432, 422, 418, 413, 413, 397)   # LFC < 0 (last value missing)
)

# Convert cut_off to a factor to ensure discrete x-axis
screening_DEseq$cut_off <- factor(screening_DEseq$cut_off, levels = screening_DEseq$cut_off)

# Plot
screening_plot <- ggplot(screening_DEseq, aes(x = cut_off)) +
  geom_line(aes(y = LFC_up, group = 1, color = "LFC > 0"), size = 1.2) +  
  geom_line(aes(y = LFC_down, group = 1, color = "LFC < 0"), size = 1.2) +
  geom_point(aes(y = LFC_up, color = "LFC > 0"), size = 3) +
  geom_point(aes(y = LFC_down, color = "LFC < 0"), size = 3) +
  scale_color_manual(values = c("LFC > 0" = "red", "LFC < 0" = "blue")) +
  labs(
    x = "Cut-off",
    y = "Number of Genes",
    title = "LFC Up and Down across Cut-offs",
    color = "LFC"
  ) +
  theme_bw() +
  ...
  # Put the little lines inside the plot not outsie 
  theme() +
  scale_y_continuous(breaks = seq(300, 1000, by = 100), limits = c(390, 900))

# save
ggsave(filename = "plots_v1/DEseq2_cut_off_screening.jpg",
       plot = screening_plot, width = 9, height = 6, dpi = 300)
...
write.xlsx(res.TB_C, file = paste0("results_v1/res_TB_vs_C_", dea_formula, "_all.xlsx"),
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)

# Significant
res.TB_C.sig <- subset(res.TB_C, ((padj < cutoff_alpha) & !is.na(padj)))

write.xlsx(res.TB_C.sig, file = paste0("results_v1/res_TB_vs_C_", dea_formula, "_padj_0.25.xlsx"),
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)
```
...
write.xlsx(res.TB_C.det, file = paste0("results_v1/res_TB_vs_C_", dea_formula, "padj_0.25_detectable.xlsx"),
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)
write.xlsx(res.TB_C.det[res.TB_C.det$padj < 0.05, ], file = paste0("results_v1/res_TB_vs_C_", dea_formula, "padj_0.05_detectable.xlsx"),
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)
write.xlsx(res.TB_C.det[res.TB_C.det$padj < 0.01, ], file = paste0("results_v1/res_TB_vs_C_", dea_formula, "padj_0.01_detectable.xlsx"),
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)
...
res.TB_C <- readxl::read_xlsx("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Results_v1/res_TB_vs_C_grouppadj_0.05_detectable.xlsx", sheet = 1)


common_genes <- c("STAT1","GBP1","IFIT3","IFI6","GCH1","RSAD2","CMPK2", "IFITM3","SERPING1","FCGR1A", "IFI27")

for (gene in x$ensembl[x$symbol %in% common_genes]) {
  gene_counts <- counts(ds.deseq, normalized = TRUE)[gene, ]
  log2_gc <- log2(gene_counts)
  
  df.box <- rbind(df.box,
                  data.frame(ds.deseq@colData[, c("id", "group", "sex", "age", "num")],
                             log2_gc,
...
ggsave(filename = paste0("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Plots_v1/BSV/BSV_top_hits_facet.jpg"), plot = p.bs, width = 11, height = 7.5, dpi = 300)
```
...
ggsave(paste0("plots/volcano/TB_Control_volcano_0.05_detectable_", "padj<0.001", ".jpg"),
       p.volcano, width = 11, height = 8, dpi = 300)
```

## Volcano plot all and detectable
```{r}
# Load necessary files
res.TB_C <- readxl::read_xlsx("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Results_v1/res_TB_vs_C_grouppadj_0.05_detectable.xlsx", sheet = 1)
...
# Add all data for volcano
all_data <- readxl::read_xlsx("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Results_v1/res_TB_vs_C_group_all.xlsx", sheet = 1)
all_data <- all_data[!is.na(all_data$padj), ]
all_data$log2FoldChange[all_data$log2FoldChange > 8] <- 8
all_data$log2FoldChange[all_data$log2FoldChange < -8] <- -8
all_data$padj[all_data$padj < 10**-3] <- 10**-3
all_data$padj[all_data$padj > 10**8] <- 10**8

# Consider only not genes from d.volcano
all_data <- all_data[!all_data$ensembl %in% d.volcano$ensembl, ]
all_data$colors <- rep("other", nrow(all_data))

all_data$shapes <- rep("nohits", nrow(all_data))
all_data$shapes[all_data$padj <= 10**-3] <- "hits"

# Join rows of all_data and d.volcano into d.volcano
d.volcano <- rbind(d.volcano, all_data)
...
p.volcano <- ggplot() + theme_bw() +
  ...
  scale_fill_manual(name = "Expression",
                    values = c("over" = "red", "other" = "grey50", "under" = "blue"),
                    guide = guide_legend(override.aes = aes(shape = 21, size = 5, alpha = 1)),
                    labels = c( "Others","Up-regulated", "Down-regulated")) +
...
#ggsave("plots/Ap_An_Volcano_detect.pdf", p.volcano, width = 10.5, height = 8, dpi = 300, device = "pdf")
ggsave(paste0("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Plots_v1/volcano/Total-TB_Control_volcano_0.05_detectable_padjless0.001", ".pdf"), #.jpg
       p.volcano, width = 11, height = 8, dpi = 300)
```
...
v2
```{r}
load("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Results_v1/02_17_updated_Rdata.RData")
...
varst.data.sig <- varst.data[rownames(varst.data) %in% genesTB$ensembl, ]
varst.data.sig <- varst.data.sig[match(genesTB$ensembl, rownames(varst.data.sig)), ]
## There is a problem with the rownames, they are not matching, so I will try to fix it later.

# Remove NA rows
varst.data.sig <- varst.data.sig[complete.cases(varst.data.sig), ]
...
ggsave("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Plots_v1/heatmap/Heatmap_samples_TB_Control_(padj_0.05)_WPGMA.pdf",
       plot = heatmap_genes, width = 6, height = 9, dpi = 300, units = "in")
#ggsave("plots/heatmap/Heatmap_samples_TB_Control_(padj_0.01)_WPGMA.jpg",
#       plot = heatmap_genes, width = 6, height = 6, dpi = 300, units = "in")
```

### 03_17_2025 Heatmaps Samples vsGenes Non-common genes included in datasets
```{r}
...
# Filter common genes
res.TB_C.noncommon <- res.TB_C[!res.TB_C$symbol %in% common_genes,]

# Consider only log2FoldChange > 2
res.TB_C.noncommon <- res.TB_C.noncommon[abs(res.TB_C.noncommon$log2FoldChange) > 2,]

nrow(res.TB_C.noncommon)

genesTB <- res.TB_C.noncommon %>%
  select("ensembl", "symbol")
...
ggsave("C:/Users/dan_1/Documents/PROJECTS/TB/DEG/Plots_v1/heatmap/Heatmap_samples_TB_Control_noncommon_genes_log2FC_meq_2_(padj_0.05)_WPGMA_v2.jpg",
       plot = heatmap_genes, width = 6, height = 9, dpi = 300, units = "in")
...
```{r}
# Load Rdata
load("C:/Users/dan_1/Documents/PROJECTS/TB/Results_v1/02_17_updated_Rdata.RData")
# Expression data
expression_data_TB <- normalized.counts.annotated[, c(20, 21, 27, 1:19)]
class(expression_data_TB)
ncol(expression_data_TB) 
colnames(expression_data_TB)[2:3] <- c("NAME", "DESCRIPTION")
expression_data_TB$DESCRIPTION <- rep(NA, nrow(expression_data_TB))

# See duplicated gene symbols
table(duplicated(expression_data_TB$NAME)) # There are 36 genes with duplicated symbols

# Make a data frame for all gene with duplications and save it
dup_names <- expression_data_TB[duplicated(expression_data_TB$NAME),"NAME"]
dup_names.df <- expression_data_TB[expression_data_TB$NAME %in% dup_names, c(1, 2)]
nrow(dup_names.df)

# Save into .tsv files
write.table(dup_names.df, file = "../DEG/Results_v2/GSEA_imput/04_02-duplicated_genes_filtering_mannualy.tsv",
            sep = "\t", row.names = FALSE, quote = FALSE)

####> filter manualy 

# Open the 04_02-duplicated_genes_filtering_mannualy.tsv file
curated_dup_names.df <- read.table("../DEG/Results_v2/GSEA_imput/04_02-duplicated_genes_filtering_mannualy.tsv",
                           header = TRUE, sep = "\t")

geneID_curated_genes <- curated_dup_names.df[curated_dup_names.df$MANTAIN_.GENE_CARDS_CURATED. == "YES", "geneID"]
geneID_removed_genes <- curated_dup_names.df[curated_dup_names.df$MANTAIN_.GENE_CARDS_CURATED. == "NO", "geneID"]

# Remove geneID_removed_genes appear in geneID column from expression_data_TB
expression_data_TB <- expression_data_TB[!expression_data_TB$geneID %in% geneID_removed_genes, ]

# Check duplicated gene symbols
table(duplicated(expression_data_TB$NAME)) # There are 0 gene with duplicated symbols

# Remove geneID column
expression_data_TB <- expression_data_TB[, -1]

# Write to a tab-separated and txt file
write.table(expression_data_TB, file = "../DEG/Results_v2/GSEA_imput/04_02-expression_data_TB.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(expression_data_TB, file = "../DEG/Results_v2/GSEA_imput/04_02-expression_data_TB.tsv",
            sep = "\t", row.names = FALSE, quote = FALSE)
...
## Write to .cls file
write(cls_content, file = "Results_v1/GSEA_imput/phenotype_TB.cls")
```
