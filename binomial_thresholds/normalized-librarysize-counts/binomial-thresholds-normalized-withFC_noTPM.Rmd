...
res.TB_C.det <- res.TB_C.sig[(abs(res.TB_C.sig$log2FoldChange) > 1) & 
                              (rownames(res.TB_C.sig) %in% detect_list$DetectGenes), ]
...
```{r filter 0}
# Filter by mean normalized counts (library size), not TPM
ds.deseq.full <- DESeqDataSetFromTximport(txi.toGene, sampledata, ~group)
ds.deseq.full <- estimateSizeFactors(ds.deseq.full)

# Get library-size normalized counts
norm_counts <- counts(ds.deseq.full, normalized = TRUE)

# Compute the mean normalized count for each gene across all samples
mean_norm_counts <- rowMeans(norm_counts)

# Set a threshold (e.g., genes with an average normalized count < 1 will be filtered out)
lib_size_threshold <- 0
keep_genes_lib <- mean_norm_counts > lib_size_threshold

cat("Number of genes BEFORE filtering:", nrow(norm_counts), "\n")
cat("Number of genes filtered out (mean normalized count <", lib_size_threshold, "):", sum(!keep_genes_lib), "\n")

# Filter the txi object accordingly
txi.filtered <- txi.toGene
txi.filtered$counts    <- txi.toGene$counts[keep_genes_lib, ]
txi.filtered$abundance <- txi.toGene$abundance[keep_genes_lib, ]
txi.filtered$length    <- txi.toGene$length[keep_genes_lib, ]

cat("Number of genes AFTER filtering:", sum(keep_genes_lib), "\n")
...
```{r plot DEGs per threshold}
binomial <- read.xlsx("./binomal_comparisons-normalized-withFC-notTPM.xlsx")
binomial
```

```{r data wranlgin}
binomial_tidy <- binomial %>% 
  mutate(Filtering.Stage = fct_inorder(Filtering.Stage))
binomial_tidy
...
plot_binomial <- ggplot(binomial_tidy, aes(x=Filtering.Stage, y=`Significant.DEGs.(padj.<.0.05)`, group=1)) +
  ...
  labs(x="Minimum Mean Normalized Count Threshold Before DESeq2",
       y="No. of DEGs\n(padj<0.05, |FC| > 1)")
plot_binomial

ggsave("./binomial_plot-normalized_withFC_noTPM2.png")
```

```{r}
daniel <- read.xlsx("./res_TB_vs_C_group_all.xlsx") %>% tibble()
daniel
```

```{r count DEGs from daniel table}
daniel_filter <- daniel %>% 
  filter(abs(log2FoldChange)>=1) %>% 
  filter(padj<=0.05)
  #distinct(symbol)
daniel_filter

write.xlsx(daniel_filter, file = "./results-daniel-filtered_FC1_padj0.05.xlsx",
           colNames = TRUE, rowNames = FALSE, append = FALSE, overwrite = TRUE)
